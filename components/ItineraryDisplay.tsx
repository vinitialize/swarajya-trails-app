...
export const ItineraryDisplay: React.FC<ItineraryDisplayProps> = ({ content, isLoading, userLocation, fortCoordinates }) => {
  ...
  const handleViewRoute = () => {
    if (!fortCoordinates || !content) return;
    setShowRouteSection(prev => !prev);
  };
  ...
  return (
    <div className="w-full bg-white dark:bg-slate-900 rounded-2xl shadow-lg ring-1 ring-black/5 dark:ring-white/10 p-6 md:p-8">
            ...
            {fortCoordinates && (
              <>
                <button
                    ...
                    aria-label="View route on Google Maps"
                >
      ...
      {/* Route Section */}
      {showRouteSection && fortCoordinates && content && (
        <RouteSection
          isOpen={showRouteSection}
          itineraryContent={content}
          fortCoordinates={fortCoordinates}
        />
      )}
